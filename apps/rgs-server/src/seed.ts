...
async function main(): Promise<void> {
	...
	if (!existingInbound) {
		...
		logger.info("seed: inbound credential created", {
			kid: MOCK_OPERATOR_API_KID,
		});
	}
	...
	if (!existingOutbound) {
		...
		logger.info("seed: outbound credential created", {
			kid: MOCK_OPERATOR_WALLET_KID,
		});
	}
	...
	const existingAdmin = await prisma.operatorUser.findUnique({
	...
	if (!existingAdmin) {
		...
		logger.info("seed: operator user created", {
			email: adminEmail,
			password: "admin123",
		});
	} else {
	...
	const existingStaff = await prisma.operatorUser.findUnique({
	...
	if (!existingStaff) {
		...
		logger.info("seed: staff user created", {
			email: staffEmail,
			password: "staff123",
		});
	} else {
	...
	if (!existingCert) {
		...
		logger.info("seed: sample certificate already exists", {
			certId: sampleCertId,
		});
	}
